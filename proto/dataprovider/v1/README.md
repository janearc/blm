...
Reads return a `Cell` directly. The invariants the contract guarantees:
...
the matching cells. `shard_hint` is optional — Schemaless mandates a shard key on a query
so it can route to a partition; this contract does not, because partitioning is the
implementer's concern, not the wire's. `limit` bounds the result set.
